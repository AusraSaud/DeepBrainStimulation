...
def Autoencoder(scaler, df):
    ...
    df1 = df[df['DBS motor outcome (1-poor, 2-good/very good)'] == 1]
    df2 = df[df['DBS motor outcome (1-poor, 2-good/very good)'] == 2]

    #bad DBS effect
    X1 = df1.iloc[:, 0:-1]
    y1 = df1.iloc[:, -1]
    #good and very good DBS outcome
    X2 = df2.iloc[:, 0:-1]
    y2 = df2.iloc[:, -1]

    #good and very good DBS outcome splitting in training and testing sets
    ...
    #good and very good DBS outcome: prediction and errors
    #training
    ...
    #Poor DBS outcome: prediction of outliers
...
def prediction_DBS(X, y, N = 10, scaler = scaler, df=df):
    ...
    for i in range(N):
        ...
        #Classes: 1 - poor DBS outcome, 2 - good and very good DBS outcome  
